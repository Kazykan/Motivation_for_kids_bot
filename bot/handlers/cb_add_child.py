...
sys.path.append("..")
from bot.statesgroup import AddOneMoreChildStatesGroup, AddOneMoreParentSGroup, AddParentStatesGroup
...
@router.callback_query(GenderCFactory.filter(), AddOneMoreChildStatesGroup.gender)
...
async def cb_add_one_more_child_phone(message: types.Message, state: FSMContext) -> None:
    ...
    if parent_number:
        first_add_child_id = ChildDB.get_first_child_id(bot_user_id=int(message.from_user.id))
